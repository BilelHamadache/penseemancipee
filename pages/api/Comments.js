...
export default async function Comments (req, res){
  ...
  console.log(graphcmsToken);
    ...
    //envoyer commentaire à GraphCMS
  ...
  //Créer la requete 'mutation' et non pas 'query' qui va créer une nouvelle ligne dans la table commentaire dans graphCMS dashboard
  const query_submit = gql`
   mutation createComment ($nom: String!, $email: String!, $comment: String!, $lien: String!)
   {
    createCommentaire(data: {nom: $nom, email: $email, contenu: $comment, post: {connect: {lien: $lien}}}) { id }
   }
  `
  //Exécuter la requete en paramètre avce un autre paramètre = data
  try
  {
  const result = await graphQLClient.request(query_submit, req.body);
